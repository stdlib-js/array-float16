...
tape( 'attached to the prototype of the main export is an `every` method for returning boolean indicating whether all elements pass a test', function test( t ) {
	t.strictEqual( hasOwnProp( Float16Array.prototype, 'every' ), true, 'has property' );
	t.strictEqual( isFunction( Float16Array.prototype.every ), true, 'has method' );
	t.end();
});
...
tape( 'the method returns `true` if operating on an empty floating-point number array', function test( t ) {
	var bool;
	var arr;
...
tape( 'the method returns `false` if one or more elements fail a test', function test( t ) {
	...
	arr = new Float16Array( [ 1.05, 1.05, 0.05, 1.05 ] );
	bool = arr.every( predicate );
